 ...
 //   pc.printf("offer %d\r\n",size);
    ...
    while(*p != 255 && p < (buf+size)) 
    {
      ...
      //         pc.printf("IP %d-%d-%d-%d\r\n",p[0],p[1],p[2],p[3]);
            ...
            case 6:  // DNS server
                memcpy(dnsaddr, p, 4);
      //           pc.printf("DNS  server %d-%d-%d-%d\r\n",p[0],p[1],p[2],p[3]);               
                break;
            case 51: // IP lease time 
       //         pc.printf("time of life %d-%d-%d-%d\r\n",p[0],p[1],p[2],p[3]);
                break;
            case 54: // DHCP server
                memcpy(siaddr, p, 4);
       //         pc.printf("DHCP server %d-%d-%d-%d\r\n",p[0],p[1],p[2],p[3]);
                break;
        }
        p += len;
    }
